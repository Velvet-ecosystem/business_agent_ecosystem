...
def build(tmp_path: Path, *, fail: bool = False):
    ...
    executor = NotificationDeliveryExecutor(jobs, drafts, deliveries, adapter, receipts)
    ...
    return coordinator, jobs, drafts, deliveries, adapter
...
def test_delivery_sends_once_and_records_provider_id(tmp_path: Path) -> None:
    coordinator, jobs, drafts, deliveries, adapter = build(tmp_path)
    seed(jobs, drafts)
...
def test_retry_with_same_key_does_not_send_twice(tmp_path: Path) -> None:
    coordinator, jobs, drafts, _, adapter = build(tmp_path)
    seed(jobs, drafts)
...
def test_provider_failure_creates_no_delivery_record(tmp_path: Path) -> None:
    coordinator, jobs, drafts, deliveries, adapter = build(tmp_path, fail=True)
    seed(jobs, drafts)
...
def test_missing_draft_blocks_delivery(tmp_path: Path) -> None:
    coordinator, jobs, _, _, _ = build(tmp_path)
