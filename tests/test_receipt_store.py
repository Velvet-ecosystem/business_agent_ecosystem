...
def test_receipts_read_in_append_order(tmp_path: Path) -> None:
    store = JsonlReceiptStore(tmp_path / "receipts.jsonl")
    first = store.append(actor="Court", decision="denied", subject_id="one", details={"reason": "test"})
    second = store.append(actor="Task Executor", decision="completed", executor="Task Executor", subject_id="two", details={"task_id": "task_0001"})
    receipts = store.read_all()
    assert [receipt.receipt_id for receipt in receipts] == [first.receipt_id, second.receipt_id]
...
def test_invalid_jsonl_line_fails_closed(tmp_path: Path) -> None:
    ...
    with pytest.raises(ValueError, match="invalid JSONL .*:2"):
        store.read_all()
...
def test_hmac_receipt_requires_matching_key(tmp_path: Path) -> None:
    ...
    receipt = signing_store.append(actor="Court", decision="approved", subject_id="small-workshop", details={"authorization_id": "auth:test"})
...
def test_required_signing_rejects_unsigned_receipt(tmp_path: Path) -> None:
    ...
    unsigned = unsigned_store.append(actor="Court", decision="denied", subject_id="small-workshop", details={"reason": "test"})
    strict_store = JsonlReceiptStore(path, signing_key=b"local-test-key", require_signing=True)
