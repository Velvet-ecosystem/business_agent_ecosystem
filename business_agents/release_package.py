...
class ReleasePackage:
    ...
    def __post_init__(self) -> None:
        for name in (
            "release_package_id", "artifact_id", "evidence_id", "inspection_id",
            "stock_eligibility_decision_id", "release_review_id", "handler_id",
        ):
...
class ReleasePackageStore:
    def __init__(self, path: str | Path) -> None:
        self.path = Path(path)
        self._storage = CompatibleLockedJsonlFile(self.path, schema="release-package", version=1)
...
def build_release_package(
    *, release_package_id: str, eligibility: StockEligibilityDecision,
    review: ReleaseReview, handler_id: str,
) -> ReleasePackage:
    ...
    if review.mutates_stock is not False or review.executes_release is not False:
        raise ValueError("release review must remain non-mutating and non-executing")
...
def calculate_release_package_digest(payload: Mapping[str, Any]) -> str:
    canonical_json = json.dumps(dict(payload), sort_keys=True, separators=(",", ":"), ensure_ascii=False)
