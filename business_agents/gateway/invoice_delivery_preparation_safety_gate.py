"""Safety gate for local invoice delivery preparation."""
...
class InvoiceDeliveryPreparationSafetyGate:
    ...
    def evaluate(self, intent: BusinessIntent) -> SafetyDecision:
        ...
        if intent.action != "prepare-invoice-delivery":
            return SafetyDecision(False, "unsupported-action")
        if intent.risk_level is not RiskLevel.HIGH:
            return SafetyDecision(False, "invoice-delivery-preparation-risk-too-low")
        if intent.approval_mode is not ApprovalMode.STRONG_HUMAN:
            return SafetyDecision(False, "strong-human-approval-required")
        allowed = {"preparation_id", "finalization_id", "invoice_id", "job_id", "prepared_by"}
        if set(intent.parameters) != allowed:
            return SafetyDecision(False, "invalid-invoice-delivery-preparation-fields")
        ...
        return SafetyDecision(True, "safe-invoice-delivery-preparation")
