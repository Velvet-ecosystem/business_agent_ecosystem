"""Safety gate for archival communication records."""
...
class CommunicationHistorySafetyGate:
    ...
    def evaluate(self, intent: BusinessIntent) -> SafetyDecision:
        ...
        if intent.action != "record-communication-reference":
            return SafetyDecision(False, "unsupported-action")
        if intent.risk_level is not RiskLevel.HIGH:
            return SafetyDecision(False, "communication-record-risk-too-low")
        ...
        if set(intent.parameters) != allowed:
            return SafetyDecision(False, "invalid-communication-record-fields")
        if intent.parameters.get("job_id") != intent.subject_id:
            return SafetyDecision(False, "job-id-subject-mismatch")
        if intent.parameters.get("direction") not in {"inbound", "outbound"}:
            return SafetyDecision(False, "invalid-direction")
        for field in allowed:
            value = intent.parameters.get(field)
            if not isinstance(value, str) or not value.strip():
                return SafetyDecision(False, f"invalid-{field.replace('_', '-')}")
        return SafetyDecision(True, "safe-communication-history-record")
